...
class Scanner {
  ...
  solve(diff: number[], r: number) {
    ...
    this.solved = true;
    this.position = diff as [number, number, number];
    // The lengths don't change
  }

  manhattanDistance(other: Scanner) {
    if (!this.solved || !other.solved) {
      throw new Error();
    }
    return (
      Math.abs(this.position[0] - other.position[0]) +
...
function main() {
  ...
  for (let x = 0; x < scanners.length; x++) {
    const xScanner = scanners[x];
    for (let y = x + 1; y < scanners.length; y++) {
      const yScanner = scanners[y];
      const distance = xScanner.manhattanDistance(yScanner);
      if (distance > answer) {
        answer = distance;
      }
    }
